...
export default function MyPage() {
  ...
  const [items, setItems] = useState([])   // 저장된 가게들
  ...
  async function load() {
    ...
    const { data: fs } = await supabase.from('folders').select('*')
      .eq('user_id', u.user.id).order('created_at')
    setFolders(fs ?? [])

    const ids = (fs ?? []).map((f) => f.id)
    if (ids.length) {
      const { data: its } = await supabase.from('saved_places')
        .select('id, folder_id, places(*)').in('folder_id', ids)
      setItems(its ?? [])
    } else {
      setItems([])
    }
  ...
  async function togglePublic(f) {
    await supabase.from('folders').update({ is_public: !f.is_public }).eq('id', f.id)
    load()
  }
  ...
  if (!user) return (
    <div className="p-6">로그인이 필요해요. <Link href="/login" className="text-blue-600 underline">로그인</Link></div>
  )
  ...
  return (
    <main className="max-w-md mx-auto p-4">
      ...
      {folders.length === 0 && (
        <p className="text-gray-500">아직 폴더가 없어요. 지도에서 가게를 저장해보세요.</p>
      )}
          ...
          return (
            <section key={f.id} className="border rounded-lg p-3">
              <div className="flex justify-between items-center">
                <h2 className="font-bold">{f.name} {f.is_public ? '🌐 공개' : '🔒 나만'}</h2>
                <div className="flex gap-2">
                  <button onClick={() => togglePublic(f)} className="text-xs text-blue-600">
                    {f.is_public ? '🔒 나만보기로' : '🌐 공개로'}
                  </button>
                  <button onClick={() => deleteFolder(f.id)} className="text-xs text-red-500">폴더 삭제</button>
                </div>
              </div>
